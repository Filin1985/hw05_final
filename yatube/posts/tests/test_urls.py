...
class PostURLTests(TestCase):
    ...
    def test_unathorized_url_exists_at_desired_location(self):
        ...
        for url, client, status in urls_cases:
            with self.subTest(url=f"""
            Проверка {url} возвращает статус отличный
            от {status} для пользователя {client}
            """):
                self.assertEqual(
    ...
    def test_urls_uses_correct_template(self):
        ...
        for address, client, template in templates_url_names:
            with self.subTest(address=address):
                if not settings.DEBUG and address == UNEXISTING_URL:
    ...
    def test_posts_list_url_redirect(self):
        ...
        for destination, url, user in cases:
            with self.subTest(destination=f"""
            {destination} возвращает отличный от
            ожидаемого результат для {user}
            """):
                if not settings.DEBUG and url == UNEXISTING_URL:
